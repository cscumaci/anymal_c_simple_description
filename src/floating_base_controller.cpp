...
{ 
    ...
    dot_dot_q_temp = Eigen::MatrixXd::Zero(6,1);  ////
        ...
        if(c_switch == 0)
        {
           ...
           // KER di Jc (18x6)
            ...
            pseudo_inverse(S_S,S_S_inv);
              Eigen::JacobiSVD<MatrixXd> svd(S_S_inv, ComputeThinU | ComputeThinV);

            cout << "singular values"<< endl << svd.singularValues() <<endl;

        Eigen::VectorXd sv = Eigen::VectorXd::Zero(6,1);
        sv = svd.singularValues();


        geometry_msgs::Twist sing_values;

        sing_values.linear.x = sv(0,0);
        sing_values.linear.y = sv(1,0);
        sing_values.linear.z = sv(2,0);
        sing_values.angular.x = sv(3,0);
        sing_values.angular.y = sv(4,0);
        sing_values.angular.z = sv(5,0);

        this-> sing_values.publish(sing_values);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        force_L.linear.x = base_des (0);
        force_L.linear.y = base_des (1);
        force_L.linear.z = base_des (2);
        force_L.angular.x = base_des (3);
        force_L.angular.y = base_des (4);
        force_L.angular.z = base_des (5);

        //RF and RH
        force_R.linear.x = q_curr(0);
        force_R.linear.y = q_curr(1);
        force_R.linear.z = q_curr(2);
        force_R.angular.x = q_curr(3);
        force_R.angular.y = q_curr(4);
        force_R.angular.z = q_curr(5);
